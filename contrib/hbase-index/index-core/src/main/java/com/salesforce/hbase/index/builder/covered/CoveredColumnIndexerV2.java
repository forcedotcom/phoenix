 ...
 * <p>
 * The family is always {@link CoveredColumnIndexCodecV2#INDEX_ROW_COLUMN_FAMILY}
 * <p>
 ...
 */
public class CoveredColumnIndexerV2 extends PhoenixIndexBuilder {
  ...
  public static void createIndexTable(HBaseAdmin admin, String indexTable) throws IOException {
    ...
    HColumnDescriptor col =
        new HColumnDescriptor(CoveredColumnIndexCodecV2.INDEX_ROW_COLUMN_FAMILY);
  ...
  public Collection<Pair<Mutation, String>> getIndexUpdateForFilteredRows(
    ...
    for (Entry<byte[], Collection<KeyValue>> batch : batches.asMap().entrySet()) {
      ...
      TreeMultimap<Long, KeyValue> timeBatch = createTimestampBatchesFromMutation(p);
      LocalTableState state = new LocalTableState(env, localTable, p);
      for (Entry<Long, Collection<KeyValue>> entry : timeBatch.asMap().entrySet()) {
        //just set the timestamp on the table - it already has all the future state
        state.setCurrentTimestamp(entry.getKey());
        this.addDeleteUpdatesToMap(updateMap, state, entry.getKey());
      }
    }
    return updateMap;
  }
